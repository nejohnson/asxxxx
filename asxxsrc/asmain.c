 ...
 *	function minit(), where passlmt is the maximum # of passes:
 *
 *	passlmt = 100;
 *	if (pass < 2) {
 *		for (i=0; i<NB; i++) {
 ...
 */
char *
filespec(int argc, char *argv[], int *i, char *p, char **nam, char **ext, int opt)
{
	char *q;
	int pFile;
 ...
 */
void
pckchr(a_uint size, int c, a_uint *v, a_uint *cnt)
{
	if ((int) hilo) {
		*cnt += 1;
		*v |= ((c & 0xFF) << 8 * (size - (*cnt)));
